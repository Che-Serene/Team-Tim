...
class SelfDrivingNode(Node):
    def __init__(self, name):
        ...
        self.classes = ['cross', 'go', 'green', 'off', 'parking', 'red', 'right', 'yellow']
        self.display = True
    ...
    def main(self):
        while self.is_running:
            ...
            if self.start:
                ...
                self.get_logger().info('횡단보도 \033[1;33m%s\033[0m' % self.crosswalk_distance)

                if self.turn_right and 135<self.crosswalk_distance:
                    
                    twist.linear.x = 0.0
                    self.mecanum_pub.publish(twist)
                    time.sleep(1)
                    self.get_logger().info("'right' sign detected, executing turn.")
                    twist.linear.x = self.normal_speed
                    twist.angular.z = 0.0
                    self.mecanum_pub.publish(twist)
                    time.sleep(0.4) # 잠시 직진

                    twist.linear.x = self.slow_down_speed
                    twist.angular.z = -0.8  # 우회전 각속도
                    self.mecanum_pub.publish(twist)
                    time.sleep(1.8) # 정해진 시간 동안 회전

                    self.turn_right = False # <-- 초기화 중요
                    self.get_logger().info("Right turn finished. Resuming lane following.")
                    self.mecanum_pub.publish(Twist()) 
                #    continue # 다음 main 루프 반복으로 넘어감
                else:
                    twist.linear.x = self.normal_speed
                    twist.angular.z = 0.0                  
                ...
                if lane_x >= 0 and not self.stop:  
                    self.get_logger().info('\033[1;33m%s\033[0m' % lane_x)
                    if lane_x > 170:  
                        self.count_turn += 1
                        if self.count_turn > 3 and not self.start_turn:
                        ...
                        if self.machine_type != 'MentorPi_Acker':
                            twist.linear.x = self.slow_down_speed
                            twist.angular.z = -0.8  # turning speed
                        else:
                            twist.angular.z = twist.linear.x * math.tan(-0.5061) / 0.145
                    else:  # use PID algorithm to correct turns on a straight road
                        twist.linear.x = self.normal_speed
                        self.count_turn = 0
                        if time.time() - self.start_turn_time_stamp > 1.5 and self.start_turn:
                            self.start_turn = False
                        if not self.start_turn:
                            self.pid.SetPoint = 60  # the coordinate of the line while the robot is in the middle of the lane
            ...
            for i in self.objects_info:
                ...
                if class_name == 'cross':  
                    ...
                    if self.count_right == 1:  # If it is detected multiple times, take the right turning sign to true
                        ...
                        self.count_right = 0
                elif class_name == 'parking':  # obtain the center coordinate of the parking sign
